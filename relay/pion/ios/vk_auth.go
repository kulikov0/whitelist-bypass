...
func loadCachedConfig(callback HeadlessCallback) *vkConfig {
	data := callback.LoadCache(vkConfigCacheKey)
	if data == "" {
		return nil
	}
	var cfg vkConfig
...
func saveCachedConfig(callback HeadlessCallback, cfg *vkConfig) {
	data, err := json.Marshal(cfg)
	if err != nil {
		return
	}
	callback.SaveCache(vkConfigCacheKey, string(data))
}
...
func RunVKAuth(joinLink string, displayName string, logFn func(string, ...any), statusFn StatusFunc, callback HeadlessCallback) (string, error) {
	...
	for attempt := 0; attempt < 5; attempt++ {
		...
		if errObj, hasErr := callResp["error"].(map[string]interface{}); hasErr {
			...
			if int(errCode) == 14 {
				...
				proxyPort := StartCaptchaProxy(captchaErr.redirectURI)
				if proxyPort == 0 {
