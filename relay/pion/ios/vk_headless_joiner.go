...
func NewVKHeadlessJoiner(logFn func(string, ...any), resolveFn ResolveFunc, statusFn StatusFunc) *VKHeadlessJoiner {
	if logFn == nil {
		logFn = log.Printf
	}
...
func (h *VKHeadlessJoiner) emitStatus(status string) {
	if h.StatusFn != nil {
		h.StatusFn(status)
	}
}

func (h *VKHeadlessJoiner) emitStatusError(msg string) {
	h.emitStatus("ERROR:" + msg)
}

func (h *VKHeadlessJoiner) resolveHost(hostname string) (string, error) {
	if h.ResolveFn != nil {
		return h.ResolveFn(hostname)
	}
...
func (h *VKHeadlessJoiner) RunWithParams(jsonParams string) {
	...
	if err := json.Unmarshal([]byte(jsonParams), &params); err != nil {
		...
		h.emitStatusError("bad params: " + err.Error())
		return
	}
	h.authParams = &params
	...
	h.emitStatus(common.StatusConnecting)

	if err := h.joinCall(); err != nil {
		h.logFn("headless: joinCall failed: %v", err)
		h.emitStatusError(err.Error())
		return
	}
	h.connectSFU()
}
...
func (h *VKHeadlessJoiner) joinCall() error {
	...
	resolvedIP, err := h.resolveHost(parsed.Hostname())
	if err != nil {
...
func (h *VKHeadlessJoiner) readLoop() {
	for {
		...
		if err != nil {
			...
			h.emitStatus(common.StatusTunnelLost)
			return
		}
		if string(msg) == "ping" {
			h.vkMu.Lock()
...
func (h *VKHeadlessJoiner) initPC() {
	...
	if err != nil {
		...
		dc.OnOpen(func() {
			...
			if mode == "dc" {
				...
				h.emitStatus(common.StatusTunnelConnected)
	...
	pc.OnConnectionStateChange(func(state webrtc.PeerConnectionState) {
		...
		if state == webrtc.PeerConnectionStateFailed {
			...
			h.emitStatusError("connection failed")
