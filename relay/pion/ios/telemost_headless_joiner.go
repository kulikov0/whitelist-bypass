...
func NewTelemostHeadlessJoiner(logFn func(string, ...any), resolveFn ResolveFunc, statusFn StatusFunc) *TelemostHeadlessJoiner {
	if logFn == nil {
		logFn = log.Printf
	}
...
func (j *TelemostHeadlessJoiner) emitStatus(status string) {
	if j.StatusFn != nil {
		j.StatusFn(status)
	}
}

func (j *TelemostHeadlessJoiner) emitStatusError(msg string) {
	j.emitStatus("ERROR:" + msg)
}

func (j *TelemostHeadlessJoiner) resolveHost(hostname string) (string, error) {
	if j.ResolveFn != nil {
		return j.ResolveFn(hostname)
	}
...
func (j *TelemostHeadlessJoiner) RunWithParams(jsonParams string) {
	...
	if err := json.Unmarshal([]byte(jsonParams), &params); err != nil {
		...
		j.emitStatusError("bad params: " + err.Error())
		return
	}
		...
		j.httpClient = &http.Client{
			...
			Transport: &http.Transport{
				...
				DialContext: func(ctx context.Context, network, addr string) (net.Conn, error) {
					...
					resolvedIP, err := j.resolveHost(host)
					if err != nil {
						return nil, err
					}
...
func (j *TelemostHeadlessJoiner) initPC() {
	...
	subPC.OnConnectionStateChange(func(state webrtc.PeerConnectionState) {
		...
		if state == webrtc.PeerConnectionStateFailed {
			...
			j.emitStatusError("subscriber connection failed")
		}
	})
	...
	pubPC.OnConnectionStateChange(func(state webrtc.PeerConnectionState) {
		...
		if state == webrtc.PeerConnectionStateConnected && j.tunnelMode == "video" && j.vp8tunnel == nil {
			...
			j.emitStatus(common.StatusTunnelConnected)
...
func (j *TelemostHeadlessJoiner) connectAndRun() {
	...
	if err != nil {
		...
		j.emitStatusError("bad media URL")
		return
	}

	hostname := parsed.Hostname()
	resolvedIP, err := j.resolveHost(hostname)
	if err != nil {
		j.logFn("telemost-joiner: ERROR: DNS resolve failed: %s", common.MaskError(err))
		j.emitStatusError("DNS resolve failed")
		return
	}
	...
	if err != nil {
		...
		j.emitStatusError("ws connect failed")
		return
	}
	j.wsMu.Lock()
	j.ws = ws
	...
	j.emitStatus(common.StatusTunnelLost)
}

// Utility functions inlined from pion package (unexported there)
