...
| `--allow-private-dst` | да | да | да | да | да | Разрешить joiner-у ходить в локальную сеть creator-а, по умолчанию такие адреса блокируются. Loopback этим флагом не открывается, для него есть `--allow-loopback` |
| `--allow-loopback` | да | да | да | да | да | Разрешить joiner-у ходить на `127.0.0.1` самого creator-а, например в локальный инбаунд xray. По умолчанию выключено, от `--allow-private-dst` не зависит. Открывает доступ ко всем сервисам, слушающим loopback на этой машине |
...
`Restart=always` держит живым процесс, но не туннель. Отдельный класс аварий
выглядит так: креатор поднят, комната занята, новые соединения принимаются —
а полезные данные не идут ни по одному из них. На телефоне это «подключено,
интернета нет», а systemd видит здоровый процесс и не вмешивается.

В [`headless/ops/`](../headless/ops/) лежит обвязка для такого режима:
шаблонные systemd-юниты (`wbstream@<инстанс>`), ежечасная ротация логов и
watchdog, который отличает оглохший туннель от простаивающего — по соотношению
`first read` и `EOF with no data read` в логе релея. На исправном туннеле
вторых не бывает вовсе, поэтому любое их появление уже само по себе сигнал.

Пороги калибровались на одном туннеле, поэтому рядом лежит `creator-watchdog-replay`:
он прогоняет те же правила по вашим старым логам и печатает, когда бы сработал
рестарт. Сверьте вывод с моментами, когда связь реально пропадала, и поднимите
порог того правила, которое дало лишнее срабатывание.

Установка и разбор правил — в [headless/ops/README.md](../headless/ops/README.md).
